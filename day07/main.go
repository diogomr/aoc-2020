...
func partOne() {
	...
	bagContent := map[string][]string{}
	for _, rule := range rules {
		r := regexp.MustCompile("[a-z]+ [a-z]+ bag")
		bags := r.FindAllStringSubmatch(rule, -1)
		bag := strings.Split(bags[0][0], " ")
		key := bag[0] + " " + bag[1]
		for i := 1; i < len(bags); i++ {
			bag = strings.Split(bags[i][0], " ")
			value := bag[0] + " " + bag[1]
			bagContent[key] = append(bagContent[key], value)
		}
	}

	result := map[string]bool{}
	space := []string{"shiny gold"}
	for len(space) > 0 {
		lookup := space[0]
		space = space[1:]

		for k, bags := range bagContent {
			for _, bag := range bags {
				if lookup == bag {
					result[k] = true
					space = append(space, k)
				}
			}
		}
	}
...
func partTwo() {
	...
	bagContent := map[string][]string{}
	for _, rule := range rules {
		keyRegex := regexp.MustCompile("[a-z]+ [a-z]+ bag")
		keyBag := strings.Split(keyRegex.FindString(rule), " ")
		key := keyBag[0] + " " + keyBag[1]

		valRegex := regexp.MustCompile("(\\d [a-z]+ [a-z]+ bag)")
		bags := valRegex.FindAllString(rule, -1)

		for i := 0; i < len(bags); i++ {
			bag := strings.Split(bags[i], " ")
			value := bag[0] + " " + bag[1] + " " + bag[2]
			bagContent[key] = append(bagContent[key], value)
		}
	}
	result := countBags("shiny gold", &bagContent)
	fmt.Println(result)
}

func countBags(bag string, bagContent *map[string][]string) int {
	result := 0
	bags := (*bagContent)[bag]
	for _, v := range bags {
		bagSplit := strings.Split(v, " ")
		bagClr := bagSplit[1] + " " + bagSplit[2]
		num, _ := strconv.Atoi(bagSplit[0])
		result += num + (num * countBags(bagClr, bagContent))
	}
	return result
}

const input = `light orange bags contain 1 dark maroon bag, 3 dim maroon bags, 5 striped green bags, 2 pale aqua bags.
