...
func partOne() {
	...
	for i, doc := range docs {
		cleanDocs[i] = strings.ReplaceAll(doc, "\n", " ")
	}
	...
	count := 0
outer:
	for _, doc := range cleanDocs {
		fields := strings.Split(doc, " ")
		titles := map[string]bool{}
		for _, field := range fields {
			fieldSplt := strings.Split(field, ":")
			titles[fieldSplt[0]] = true
		}
...
outer:
	for _, doc := range cleanDocs {
		...
		for _, field := range fields {
			fieldSplt := strings.Split(field, ":")
			titles[fieldSplt[0]] = fieldSplt[1]
		}
		for _, field := range mandatoryFields {
			value := titles[field]
			if value == "" {
			...
			case "hgt":
				...
				r := regexp.MustCompile("^(?P<num>\\d+)(?P<unit>cm|in)$")
				matches := r.FindStringSubmatch(value)
				if len(matches) != 3 {
					continue outer
				}

				numIdx := r.SubexpIndex("num")
				unitIdx := r.SubexpIndex("unit")

				switch matches[unitIdx] {
				case "cm":
					// 	If cm, the number must be at least 150 and at most 193.
					hgt, _ := strconv.Atoi(matches[numIdx])
					if hgt < 150 || hgt > 193 {
						continue outer
					}
				case "in":
