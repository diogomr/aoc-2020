...
func partOne() {
	...
	for _, line := range lines {
		...
		ocur := lineSplt[0]
		ocurSplit := strings.Split(ocur, "-")
		minOcur, _ := strconv.Atoi(ocurSplit[0])
		maxOcur, _ := strconv.Atoi(ocurSplit[1])
		char := string(lineSplt[1][0])
		password := lineSplt[2]

		ocurCount := strings.Count(password, char)
		if ocurCount >= minOcur && ocurCount <= maxOcur {
			count++
		}
	}
...
func partTwo() {
	...
	for _, line := range lines {
		...
		ocur := lineSplt[0]
		ocurSplit := strings.Split(ocur, "-")
		firstIdx, _ := strconv.Atoi(ocurSplit[0])
		secondIdx, _ := strconv.Atoi(ocurSplit[1])
		char := string(lineSplt[1][0])
		password := lineSplt[2]

		firstEql := firstIdx-1 < len(password) && char == string(password[firstIdx-1])
		secondEql := secondIdx-1 < len(password) && char == string(password[secondIdx-1])

		if (firstEql && !secondEql) || (!firstEql && secondEql) {
			count++
		}
	}
