...
func partOne() {
	...
	world := make([][]rune, len(lines))
	for i, line := range lines {
		for _, char := range line {
			world[i] = append(world[i], char)
		}
	}

	for {
		newWorld := cloneWorld(world)
...
func cloneWorld(original [][]rune) [][]rune {
...
func areEqual(world1 [][]rune, world2 [][]rune) bool {
	for i := range world1 {
...
func countOccAdj(i int, j int, world [][]rune) int {
	count := 0
	delta := []int{-1, 0, 1}
	for _, di := range delta {
		for _, dj := range delta {
			...
			if i+di < 0 || i+di >= len(world) {
				continue
			}
			if j+dj < 0 || j+dj >= len(world[0]) {
				continue
			}
			if world[i+di][j+dj] == '#' {
				count++
...
func countVisibleOcc(i int, j int, world [][]rune) int {
	count := 0

	countMatrix := [3][3]bool{}

	delta := []int{-1, 0, 1}
	mul := 1
	for len(world) > mul || len(world[0]) > mul {
		for _, di := range delta {
			dim := (di * mul) + i
			if dim < 0 || dim >= len(world) {
				continue
			}
			for _, dj := range delta {
				if di == 0 && dj == 0 {
				...
				djm := (dj * mul) + j
				if djm < 0 || djm >= len(world[0]) {
					continue
				}
				if world[dim][djm] != '.' && !countMatrix[di+1][dj+1] {
					if world[dim][djm] == '#' {
						count++
					}
					countMatrix[di+1][dj+1] = true
				}
			}
		}
		mul++
